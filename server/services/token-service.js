...
class TokenService {
    ...
    async saveToken(userId, refreshToken){
        ...
        if(tokenData){
            await db.query('UPDATE tokens SET refreshtoken = $1 WHERE iduser = $2 RETURNING *', [refreshToken, userId]);
        }
    ...
    async removeToken(refreshToken){
        const tokenData = await db.query('DELETE FROM tokens WHERE refreshtoken = $1', [refreshToken]); tokenModel.deleteOne({refreshToken})
        return tokenData;
    }
